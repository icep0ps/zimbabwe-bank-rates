...
class Extractor {
  ...
  static genarateRates(data) {
    ...
    const textNodes = data.Pages[0].Texts.filter(
      (node) => !whitespace.test(node.R.T),
    );
    ...
    const currencies = filteredCurrencies.filter(
      (text) => !text.match(isBidOrAsk),
    );
    ...
    for (let i = currencies.length; i >= 0; i--) {
      const rate = ungroupedRates.splice(
        0,
        Math.ceil(ungroupedRates.length / i),
      );
